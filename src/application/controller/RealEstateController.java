...
public class RealEstateController implements Initializable {
  ...
  private final String ERROR_ALL = "Bitte geben füllen Sie zuerst alle Pflichteingabefleder aus.";
  ...
  public void realEstateClicked() {
    ...
    calcAdditionalCostsPerSquareMeter();
    calcSquareMeterPriceWarm();
    calcPriceCold();
    calcPriceWarm();
  }
  ...
  public void save() {
    if (isValid()) {
      realEstateService.saveOrUpdate(new RealEstate(
              "".equals(id.getText()) ? 0 : Integer.parseInt(id.getText()),
              objectNumber.getText(),
              "privat".equals(objectType.getValue()) ? ObjectTypes.PRIVATE : ObjectTypes.COMMERCIAL,
  ...
  public void clear() {
    ...
    notes.clear();
    additionalCostsPerSquareMeter.setText(NA);
    squareMeterPriceWarm.setText(NA);
    priceCold.setText(NA);
    priceWarm.setText(NA);
    errorText.setText("");
  ...
  private void calcAdditionalCostsPerSquareMeter() {
    try {
      additionalCostsPerSquareMeter.setText(String.valueOf(Double.parseDouble(additionalCosts.getText())/Double.parseDouble(squareMeters.getText())));
    } catch (Exception e) {
      additionalCostsPerSquareMeter.setText(NA);
    }
    calcSquareMeterPriceWarm();
  ...
  private void calcSquareMeterPriceWarm() {
    try {
      squareMeterPriceWarm.setText(String.valueOf(Double.parseDouble(squareMeterPriceCold.getText())+Double.parseDouble(additionalCostsPerSquareMeter.getText())));
    } catch (Exception e) {
      squareMeterPriceWarm.setText(NA);
    }
    calcPriceWarm();
  ...
  private void calcPriceCold() {
    try {
      priceCold.setText(String.valueOf(Double.parseDouble(squareMeters.getText())*Double.parseDouble(squareMeterPriceCold.getText())));
    } catch (Exception e) {
      priceCold.setText(NA);
    }
  }

  private void calcPriceWarm() {
    try {
      priceWarm.setText(String.valueOf(Double.parseDouble(squareMeters.getText())*Double.parseDouble(squareMeterPriceWarm.getText())));
    } catch (Exception e) {
      priceWarm.setText(NA);
    }
  }
  ...
  @FXML
  private void onAdditionalCosts() {
    calcAdditionalCostsPerSquareMeter();
  }

  @FXML
  private void onSquareMeters() {
    calcAdditionalCostsPerSquareMeter();
    calcPriceCold();
    calcPriceWarm();
  }
  ...
  @FXML
  private void onSquareMeterPriceCold() {
    calcSquareMeterPriceWarm();
    calcPriceCold();
  }
}
